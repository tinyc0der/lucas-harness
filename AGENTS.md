...
Follow [CONTRIBUTING.md](CONTRIBUTING.md#adding-a-new-skill) for new skills and
its [modification guidance](CONTRIBUTING.md#modifying-existing-skills) for
updates. [docs/skill-anatomy.md](docs/skill-anatomy.md) is the canonical
structure and validation contract. Keep this rules file as a pointer instead
of duplicating those instructions here.

Skills are Markdown-first: each lives at
`skills/<kebab-case-name>/SKILL.md`. Add supporting files or `scripts/` only
when the skill needs them. Per-skill zip archives are not part of the
repository contract; do not create or update them.
