# Using lucas-harness with Gemini CLI
...
gemini skills install /path/to/lucas-harness/skills/
```

**Install for a specific workspace only:**

```bash
gemini skills install /path/to/lucas-harness/skills/ --scope workspace
```
...
cat /path/to/lucas-harness/skills/incremental-implementation/SKILL.md > GEMINI.md
echo -e "\n---\n" >> GEMINI.md
cat /path/to/lucas-harness/skills/code-review-and-quality/SKILL.md >> GEMINI.md
```
...
To replicate the `lucas-harness` experience from other tools, you can configure a `SessionStart` hook that reminds you of the available skills or loads a meta-skill.
...
The repo ships 8 slash commands under `.gemini/commands/`: 7 lifecycle commands and the `/webperf` specialist audit. Gemini CLI auto-discovers them when you run from the project root.
...
| `/ship` | Parallel launch review and decision; `shipping-and-launch` delegates GO-only closeout to `memory-management` |
