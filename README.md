...
Don't know which command to reach for? The **`using-lucas-harness`** meta-skill doubles as a router (see its "Routing an Incoming Ticket" section): it classifies any incoming ticket from its content — Epic, Feature, Task, Bug, Incident, Migration, Improvement (refactor/perf), Spike, or Chore — and dispatches the correctly-calibrated flow over the commands above, skipping the steps that ticket type doesn't need and stopping for a human only at the critical gates. It activates by description rather than a slash command — just describe the ticket and let it route. Classification is content-based (a `bug:` label is a hint, not a command). It also accepts a raw goal rather than a scoped ticket — broad or fuzzy intent ("support SSO", "make onboarding smoother") is refined through the Define phase (`interview-me` / `idea-refine`) before classification, and non-actionable input is triaged out before any flow starts.
...
```
/plugin marketplace add tinyc0der/lucas-harness
/plugin install lucas-harness@lucas-harness
```
...
> ```bash
> /plugin marketplace add https://github.com/tinyc0der/lucas-harness.git
> /plugin install lucas-harness@lucas-harness
> ```
...
claude --plugin-dir /path/to/lucas-harness
```
...
```bash
agy plugin install https://github.com/tinyc0der/lucas-harness.git
```
...
```bash
git clone https://github.com/tinyc0der/lucas-harness.git
agy plugin install ./lucas-harness
```
...
```bash
gemini skills install https://github.com/tinyc0der/lucas-harness.git --path skills
```

**Install from a local clone:**

```bash
gemini skills install ./lucas-harness/skills/
```
...
AI coding agents default to the shortest path - which often means skipping specs, tests, security reviews, and the practices that make software reliable. Lucas Harness gives agents structured workflows that enforce the same discipline senior engineers bring to production code.
